...
%     end
    if do.seg_track==1
        ...
        if i==frames_do_now(1)
            p.channels=getting_channels(p,out2.Lc,do);
            all_data=cell(length(p.channels),1);
        end
        all_data=tracking_main_low_memory_2023_04_05_v1(p,do,all_data,out_data.Lc,out_data.rreg,out_data.yreg);
